...
def build_mesh_into_scene(mesh, scene, transform: np.ndarray | None = None) -> list[Command]:
    ...
    which raises `ValueError` -- uncaught here, so a degenerate transform
    aborts the walk partway through, with the commands built so far already
    applied to `scene` but never returned to any caller for undo. No
    generator or transform this milestone uses triggers that path; it is a
    documented sharp edge for a future caller supplying its own transform,
    not a handled case.
    """
