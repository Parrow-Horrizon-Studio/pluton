...
- **No C++ changes in this milestone.** `ctest` must stay at 135. No `pip install -e .` rebuild is needed.
...
Expected: schema 9, and both versions `0.13.0` once the controller has bumped `pyproject.toml`.
