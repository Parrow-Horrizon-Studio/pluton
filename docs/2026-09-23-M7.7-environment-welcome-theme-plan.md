...
Expected: PASS, 11 tests (five plain, plus two parametrized over three presets).
...
**Files:**
- Modify: `python/pluton/document.py` (`DocumentSettings`)
...
**Controller ruling carried into this task (pre-flight):** `DocumentSettings.environment` was originally Task 3's, but `document_to_dict` here reads `doc.environment`, so this task could not pass its own tests without it. The field moves here, with the codec that needs it. Task 3 keeps the renderer, the View menu and the `resolve_batch_sides` work.

**Interfaces:**
- Consumes: `Environment`, `LEGACY_ENVIRONMENT` from `pluton.viewport.environment`.
- Produces: `DocumentSettings.environment` property and `DocumentSettings.set_environment(env)`; `environment_to_dict(env) -> dict`; `environment_from_dict(d: dict | None) -> Environment`; `LoadedDocument` gains a fifth field `environment: Environment` **after** `style`; `document_to_dict` emits an `"environment"` key; `SCHEMA_VERSION == 9`.
...
In `python/pluton/document.py`, update the module docstring's first line to mention the environment, then:
...
- [ ] **Step 4: Add the codec functions**
...
- [ ] **Step 5: Wire it into the document, and add the LoadedDocument field**
...
```bash
cd /f/dev/00_Parrow-Horrizon-Studio/pluton && git add python/pluton/document.py python/pluton/io/document_codec.py python/pluton/io/pluton_file.py tests/test_document_codec.py && git commit -m "feat(io): persist the environment, schema_version 9
...
- Test: `tests/test_environment_background.py` (create)

**Interfaces:**
- Consumes: `Environment`, `DEFAULT_ENVIRONMENT`, `PRESETS`, `preset_key`, the three `*_KEY` constants from `pluton.viewport.environment`; `LoadedDocument.environment` and `DocumentSettings.environment` / `set_environment` from Task 2.
- Produces: `SceneRenderer.set_environment(env)`; `ViewportWidget.set_environment(env)`; `resolve_batch_sides(..., *, bg: tuple[float, float, float], ...)` with `bg` **required**; `MainWindow._set_environment(key)` and `MainWindow._sync_environment_ui()`; action ids `view_env_sky_ground`, `view_env_plain_white`, `view_env_studio` in group `ENVIRONMENT_GROUP = "environment"`; `_reset_document(..., *, environment)` keyword-only and required.

- [ ] **Step 1: Write the failing tests**

Create `tests/test_environment_background.py`:
...
Run: `cd /f/dev/00_Parrow-Horrizon-Studio/pluton && .venv/Scripts/python -m pytest tests/test_environment_background.py -v`

Expected: FAIL. The `resolve_batch_sides` calls fail with `TypeError: resolve_batch_sides() got an unexpected keyword argument 'bg'`.

- [ ] **Step 3: Thread the background through the renderer**
...
- [ ] **Step 4: Add the viewport passthrough**
...
- [ ] **Step 5: Add the three View menu actions**
...
- [ ] **Step 7: Fix the test files this task breaks**
...
```bash
cd /f/dev/00_Parrow-Horrizon-Studio/pluton && .venv/Scripts/python -m pytest tests/test_environment_background.py tests/test_tag_color.py tests/test_textured_shading.py tests/test_translucent_pass.py tests/test_two_sided_shading.py tests/test_main_window_scenes.py tests/test_document_codec.py -v
```

Expected: PASS. Do not pipe this through `tail`.

- [ ] **Step 9: Run the lint gates**
...
```bash
cd /f/dev/00_Parrow-Horrizon-Studio/pluton && .venv/Scripts/python -m pytest tests/test_environment_ink.py tests/test_environment.py tests/test_environment_background.py -v
```
